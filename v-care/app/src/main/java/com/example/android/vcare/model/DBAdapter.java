...
public class DBAdapter {
	...
	public static final String TEMP_DATA_TABLE = "tbl_temp_data";
	...
	private static final String USER_CREATE = "create table tbl_user(_id integer primary key autoincrement, user_name text not null, user_imei text not null, user_message text not null, user_mail text not null, user_mobile text not null);";
	private static final String DEVICE_CREATE = "create table tbl_device(_id integer primary key autoincrement, device_name text, device_email text, device_regid text not null, device_imei text not null);";
	private static final String TEMP_DATA_CREATE = "create table tbl_temp_data(_id integer primary key autoincrement, user_name text not null, user_imei text not null, user_message text not null, user_mail text not null, user_mobile text not null);";
  ...
  /********************** Main Database creation INNER class ********************/
	...
	public static void addDeviceData(String DeviceRegID,String DeviceIMEI) {
	    try{
    ...
    public static void addUserData(UserData uData) {
    	try{
				...
				String mobile  = sqlEscapeString(uData.getMobile());
				ContentValues cVal = new ContentValues();
				cVal.put(KEY_USER_IMEI, imei);
				cVal.put(KEY_USER_NAME, name);
				cVal.put(KEY_USER_MESSAGE, message);
				cVal.put(KEY_USER_MAIL, email);
				cVal.put(KEY_USER_MOBILE, mobile);
    ...
    public static void addTempData(UserData uData) {
    	try{
		    	final SQLiteDatabase db = open();
				...
				cVal.put(KEY_USER_MAIL, email);
				cVal.put(KEY_USER_MOBILE, mobile);
				db.insert(TEMP_DATA_TABLE, null, cVal);
		        db.close(); // Closing database connection
	    } catch (Throwable t) {
			Log.i("Database", "Exception caught: " + t.getMessage(), t);
		}
    ...
    public static UserData getUserData(int id) {
        ...
        Cursor cursor = db.query(USER_TABLE, new String[] { KEY_ID,
        		KEY_USER_NAME, KEY_USER_IMEI,KEY_USER_MESSAGE,KEY_USER_MAIL,KEY_USER_MOBILE}, KEY_ID + "=?",
    ...
    public static List<UserData> getAllUserData() {
        ...
        if (cursor.moveToFirst()) {
            do {
            	...
            	data.setMobile(cursor.getString(5));
    ...
    public static List<UserData> getDistinctUser() {
    	List<UserData> contactList = new ArrayList<UserData>();
        // Select All Query
        String selectQuery = "SELECT  distinct(user_imei),user_name,user_message,user_mail,user_mobile FROM " + USER_TABLE+" ORDER BY "+KEY_ID+" desc";
        ...
        if (cursor.moveToFirst()) {
            do {
            	...
            	data.setName(cursor.getString(1));
            	data.setMessage(cursor.getString(2));
    ...
    // Getting All user data
    public static List<UserData> getAllTempData() {
        List<UserData> contactList = new ArrayList<UserData>();
        // Select All Query
        String selectQuery = "SELECT  * FROM " + TEMP_DATA_TABLE+" ORDER BY "+KEY_ID+" desc";
    ...
    public static void deleteUserData(String imei){
    	try{
    		final SQLiteDatabase db = open();
    	
    	db.delete(USER_TABLE, "user_imei='"+imei+"'", null);
    	db.close();
    	}
    	catch (Throwable t) {
			Log.i("Database", "Exception caught: " + t.getMessage(), t);
    ...
    public static void deleteAllTempData(){
    	try{
    		final SQLiteDatabase db = open();
    	
    	db.delete(TEMP_DATA_TABLE, null, null);
    	db.close();
    	}
    	catch (Throwable t) {
			Log.i("Database", "Exception caught: " + t.getMessage(), t);
